...
class TechUpdatePost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return frappe.db.get_value('Tech Update Category', self.blog_category,
	...
	def validate(self):
		super(TechUpdatePost, self).validate()
		...
		# update posts
		frappe.db.sql("""update `tabTech Updater` set posts=(select count(*) from `tabTech Update Post`
			where ifnull(blogger,'')=`tabTech Updater`.name)
	...
	def get_context(self, context):
		...
		if self.blogger:
			context.blogger_info = frappe.get_doc("Tech Updater", self.blogger).as_dict()
		...
		context.category = frappe.db.get_value("Tech Update Category",
			context.doc.blog_category, ["title", "route"], as_dict=1)
		context.parents = [{"name": _("Home"), "route":"/"},
			{"name": "Tech Update", "route": "/tech_update"},
			{"label": context.category.title, "route":context.category.route}]

def get_list_context(context=None):
	frappe.msgprint("Called")
	list_context = frappe._dict(
		template = "templates/includes/tech_update.html",
	...
	if category:
		...
		blogger = frappe.db.get_value("Tech Updater", {"name": frappe.local.form_dict.blogger}, "full_name")
	...
	list_context.update(frappe.get_doc("Tech Update Settings", "Tech Update Settings").as_dict(no_default_fields=True))
	return list_context

def get_children():
	return frappe.db.sql("""select route as name,
		title from `tabTech Update Category`
		where published = 1
		and exists (select name from `tabTech Update Post`
			where `tabTech Update Post`.blog_category=`tabTech Update Category`.name and published=1)
		order by title asc""", as_dict=1)

def clear_blog_cache():
	for blog in frappe.db.sql_list("""select route from
		`tabTech Update Post` where ifnull(published,0)=1"""):
		clear_cache(blog)

	clear_cache("writers")

def get_blog_category(route):
	return frappe.db.get_value("Tech Update Category", {"name": route}, "title") or route
...
def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
	...
	query = """\
		select
			t1.title, t1.name,t1._user_tags, t1.blog_category, t1.route, t1.published_on,
				...
				(select count(name) from `tabCommunication`
					where
						...
						and reference_doctype='Tech Update Post'
						and reference_name=t1.name) as comments
		from `tabTech Update Post` t1, `tabTech Updater` t2
	...
	for post in posts:
		...
		post.category = frappe.db.get_value('Tech Update Category', post.blog_category,
